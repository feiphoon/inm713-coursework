...
parser.add_argument(
    "--ontology_file", type=str, default=None, help="The input ontology for embedding"
)
parser.add_argument(
    "--embedding_dir", type=str, default=None, help="The output embedding directory"
)
parser.add_argument(
    "--config_file", type=str, default="default.cfg", help="Configuration file"
)
...
if "embedding_dir" not in config["BASIC"]:
    config["BASIC"]["embedding_dir"] = os.path.join(
        config["DOCUMENT"]["cache_dir"], "output"
    )

start_time = time.time()
if (
    (
        "ontology_projection" in config["DOCUMENT"]
        and config["DOCUMENT"]["ontology_projection"] == "yes"
    )
    or "pre_entity_file" not in config["DOCUMENT"]
    or "pre_axiom_file" not in config["DOCUMENT"]
    or "pre_annotation_file" not in config["DOCUMENT"]
):
    print("\n Access the ontology ...")
...
if (
    ...
    onto_projection_file = os.path.join(
        config["DOCUMENT"]["cache_dir"], "projection.ttl"
    )
...
if "pre_entity_file" in config["DOCUMENT"]:
    entities = [
        line.strip() for line in open(config["DOCUMENT"]["pre_entity_file"]).readlines()
    ]
else:
...
if "pre_annotation_file" in config["DOCUMENT"]:
    ...
    with open(
        os.path.join(config["DOCUMENT"]["cache_dir"], "annotations.txt"), "w"
    ) as f:
...
# Save all_doc
with open(
    os.path.join(config["DOCUMENT"]["cache_dir"], "document_sentences.txt"), "w"
) as f:
...
start_time = time.time()
if "pre_train_model" not in config["MODEL"] or not os.path.exists(
    config["MODEL"]["pre_train_model"]
):
    print("\nTrain the language model ...")
    model_ = gensim.models.Word2Vec(
        all_doc,
        size=int(config["MODEL"]["embed_size"]),
